...
int read_knots(const int KNOTS)
{
    ...
    /*
    Эта функция запрашивает у пользователя количество спичек для размещения в
    этой кучки и проверяет, находится ли количество спичек, введенных для кучки,
    в диапазоне от 1 до максимального разрешенного количества объектов. array []
    - это массив кучек. size-это размер массива. MATCHS-это максимальное
    количество спичек на стержень.
    */
    void procedure(int array[], int size, const int MATCHS)
    {
        int n;
    ...
    /*
    Эта функция вычисляет общее количество спичек в массиве кучек, пересекает
    массив кучек и вызывает вспомогательную процедуру для отображения каждой
    полной строки.
    */
    void draw_knots(int array[], int size)
    {
    ...
    */
    ...
    Статистика состоит из 1) кучек с наименьшим количеством спичек, 2) кучек с
    наибольшим количеством спичек и 3) среднего количества спичек в кучки с
    учетом только кучек с спичками. size-это размер массива. MATCHS-это
    максимальное количество спичек на стержень.
    */
    void statistics(int array[], int size)
    {
        smallest(array, size);
    ...
    */
    ...
    /*
    Эта функция отображает среднее количество объектов на стержень с учетом
    только стержней с объектами.
    */
    void average(int array[], int size)
    {
        int sum(0);
    ...
    /*
    Эта функция возвращает true, если все стержни имеют нулевой объект, и
    ...
    */
    ...
    /*
    Эта функция реализует шаг (vi) алгоритма, на котором текущий игрок делает
    свой следующий ход.
    */
    void next_move(
            int array[],
    ...
    /*
    Эта функция предлагает игроку выбрать кучку (идентификатор кучки), а затем
    ...
    */
    int choose_knot(int player)
    {
        int knot_id;
        cout << "Игрок (" << player
             << ") :На какой кучки вы хотели бы играть? ";
        cin >> knot_id;
        return (knot_id);
    }

    /*
    Эта функция проверяет кучку, выбранный этим игроком, т. е. выбранная качка
    должен находиться в диапазоне от 0 до n-1, где n-общее количество кучек.
    */
    void validate_knot(int player, int& knot_chosen, int size)
    {
        while (knot_chosen < 0 || knot_chosen > size - 1) {
            cout << "Неверный номер кучки. Пожалуйста, попробуйте еще раз."
                 << endl;
            knot_chosen = choose_knot(player);
        }
    }
    ...
    */
    ...
    /*
    Эта функция запрашивает и считывает, сколько объектов нужно удалить из
    выбранного стержня.
    */
    int numofmatchs_to_remove(int matchs, int knot_chosen)
    {
    ...
    /*
    Эта функция проверяет, является ли число удаляемых объектов положительным и
    не превышает ли оно число объектов на выбранном стержне.
    */
    void validate_matchs(
            int array[], int knot_chosen, int& matchs_removed, int matchs)
    {
        while (matchs_removed < 1 || matchs_removed > matchs) {
            ...
            matchs_removed
                    = numofmatchs_to_remove(array[knot_chosen], knot_chosen);
        }
    }

    /*
    Эта функция изменяет массив стержней, вычитая указанное количество спичек из
    выбранной кучки.
    */
    void move_matchs(int array[], int knot_chosen, int matchs_removed)
    {
